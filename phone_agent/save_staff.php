...
$date = new DateTime();
	...
	if(!empty($_POST)){
		...
		for($x = 0 ; $x < count($question); $x++){
			...
			$getAnswer = mysqli_query($db, "SELECT correct FROM ts_phoneagent_question_choices WHERE question_id='$question_id'");
		...
		$checkEmail = $db->query($crud->getOrData('ts_phoneagent_users', array("email"), array($_SESSION['email'])));
		$checkBlackList = $db->query($crud->getOrData('ts_phoneagent_blacklist', array("email"), array($_SESSION['email'])));
...
//		exit;
		if ($checkEmail->num_rows > 0) {
			...
			if($checkBlackList->num_rows > 0) {
				...
				$query = $crud->add('ts_phoneagent_users', $data);
				...
				if($result){
					...
					$answer_data = array(
						'phoneagent_user_id' => $insert_id,
						'question_details'  => json_encode($question),
						'answer_details'    => json_encode($answers),
						//'date_created'      => date("Y-m-d H:i:s")
						'date_created' => date("Y-m-d H:i:s" , $date->getTimestamp())
					);
					$db->query($crud->add('ts_phoneagent_answers', $answer_data));
...
//                    if (isset($_COOKIE['sourceURL'])) {
//                        unset($_COOKIE['sourceURL']);
//                        setcookie('sourceURL', '', -1);
//                    }
