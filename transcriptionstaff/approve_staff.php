...
$crud = new Crud();
	...
	if($_POST['status'] == 0){
		...
		if($result){
			...
			$queryReject = $crud->add('ts_blacklist', $data);
			$resultReject = $db->query($queryReject);
			
			if($resultReject){
				$recipient = $_POST['email'];
				$subject = "Transcription Staff Registration";
				$bodyHtml = "<p>Dear ".$_POST['fullname'].",<br/><br/>Thank you for your application,  unfortunately  we can not accept your application at this moment.<br/><br/>Yours Sincerely,<br/><br/>Management<br/>TranscriptionStaff.com</p>";
		...
		$getTranscriberEmailContent = mysqli_query($db, "SELECT * FROM ts_approve_email_template WHERE type='1'");
		...
		$getTCEmailContent = mysqli_query($db, "SELECT * FROM ts_approve_email_template WHERE type='2'");
		$TCResult = mysqli_fetch_all($getTCEmailContent, MYSQLI_ASSOC);
		
		$TCContent = str_replace("%24name",$_POST['fullname'],$TCResult[0]['content']);
		$TCContent = str_replace("%24email",$_POST['email'],$TCContent);
		$TCContent = htmlentities(urldecode($TCContent));
		...
		if($_POST['account_type'] == 1){
			...
			$bodyHtml = "<p style='white-space: pre-line;'>" . $TCContent . "</p>";
		}
