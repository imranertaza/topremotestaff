...
if(!isset($_GET['tab']) || (isset($_GET['tab']) && $_GET['tab'] == "pending")) {
	$getBookKeepingData = mysqli_query($db, "SELECT * FROM ts_bookkeeping_users WHERE status='0' ORDER BY date_created DESC");
	$BookKeepingResult = mysqli_fetch_all($getBookKeepingData, MYSQLI_ASSOC);
}
if(isset($_GET['tab']) && $_GET['tab'] == "approved") {
	$getBookKeepingApproved = mysqli_query($db, "SELECT * FROM ts_bookkeeping_users WHERE status='1' ORDER BY date_updated DESC");
	$approveBookKeepingResult = mysqli_fetch_all($getBookKeepingApproved, MYSQLI_ASSOC);
}
if(isset($_GET['tab']) && $_GET['tab'] == "mc_questions") {
	$getQuestions = mysqli_query($db, "SELECT * FROM ts_bookkeeping_questions WHERE status='1' ORDER BY date_created DESC");
	$questionResult = mysqli_fetch_all($getQuestions, MYSQLI_ASSOC);
}
if(isset($_GET['tab']) && $_GET['tab'] == "email_template") {
	$getApproveEmailTemplate = mysqli_query($db, "SELECT * FROM ts_bookkeeping_approve_email_template");
	$approveEmailTemplate = mysqli_fetch_all($getApproveEmailTemplate, MYSQLI_ASSOC);

	$getFollowupEmailTemplate = mysqli_query($db, "SELECT * FROM ts_bookkeeping_followup_email_template");
...
<body>
	...
	<h2>Bookkeeping</h2>
	<div id="BookKeeping" class="tabcontent p-1" style="display: block;">
		<div class="tab">
		<?php 
		if(!empty($_GET)){ 
				...
				<table cellspacing="0">
					<thead>
						<tr>
							...
							<th><button class="btn-danger" onclick="showDeleteAllPendingBookKeepingModal()">REMOVE ALL</button></th>
						</tr>
					</thead>
					<tbody>
							...
							<?php for($x = 0; $x < count($BookKeepingResult); $x++){ ?>
								<tr>	
									...
									<td>
										...
										<div id="viewTestResultModal<?php echo $BookKeepingResult[$x]['id']; ?>" class="modal">
											<div class="modal-content" style="width: 45%;">
												 <div class="modal-header">
													<span class="close" style="top: -10px;" onclick="closeModal(<?php echo "viewTestResultModal" . (string)$BookKeepingResult[$x]['id']; ?>)">×</span>
													<h4 style="margin:0">Test Result: Multiple Choice</h4>
												 </div>
												 <div class="modal-body" style="padding-bottom:24px">
													<p style="margin-top:0;">Score: <span style="color:#18AACF;font-weight:bold;"><?php echo $BookKeepingResult[$x]['test_score']; ?>%</span></p>
													<div id="test_result_<?php echo $BookKeepingResult[$x]['id']; ?>"></div>
												</div>
											</div>
										</div>
									</td>
									<td>
										<button class="btn-success" onclick="showModal(<?php echo "approveBookKeepingmodal" . (string)$BookKeepingResult[$x]['id']; ?>)">APPROVE</button>
										<button class="btn-danger" onclick="showModal(<?php echo "declineBookKeepingmodal" . (string)$BookKeepingResult[$x]['id']; ?>)">DECLINE</button>
										<div id="approveBookKeepingmodal<?php echo $BookKeepingResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "approveBookKeepingmodal" . (string)$BookKeepingResult[$x]['id']; ?>)">×</span>
												 ...
												 <div class="modal-body">
														<form method="post" action="approve_BookKeeping_staff.php" style="display:inline">
															...
															<input type="hidden" name="email" value="<?php echo $BookKeepingResult[$x]['email']; ?>">
										...
										<div id="declineBookKeepingmodal<?php echo $BookKeepingResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "declineBookKeepingmodal" . (string)$BookKeepingResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to decline?</h3>
												 </div>
												 <div class="modal-body">
													<form method="post" action="approve_BookKeeping_staff.php">
														<input type="hidden" name="id" value="<?php echo $BookKeepingResult[$x]['id']; ?>">
														<input type="hidden" name="skype" value="<?php echo $BookKeepingResult[$x]['skype']; ?>">
														<input type="hidden" name="email" value="<?php echo $BookKeepingResult[$x]['email']; ?>">
														<input type="hidden" name="paypal" value="<?php echo $BookKeepingResult[$x]['paypal']; ?>">
														<input type="hidden" name="fullname" value="<?php echo $BookKeepingResult[$x]['fullname']; ?>">
														<input type="hidden" name="status" value="0">
														<button type="submit" class="btn-danger" style="font-size:1rem;">Reject</button>
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "declineBookKeepingmodal" . (string)$BookKeepingResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									<td>
										<button class="btn-danger" onclick="showModal(<?php echo "deletePendingBookKeepingModal" . (string)$BookKeepingResult[$x]['id']; ?>)">REMOVE</button>
										<div id="deletePendingBookKeepingModal<?php echo $BookKeepingResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "deletePendingBookKeepingModal" . (string)$BookKeepingResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to remove?</h3>
												 </div>
												 <div class="modal-body">
													<form method="post" action="remove_BookKeeping_staff.php">
														<input type="hidden" name="id" value="<?php echo $BookKeepingResult[$x]['id']; ?>">
														<input type="hidden" name="check" value="1">
														<button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "deletePendingBookKeepingModal" . (string)$BookKeepingResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									<td>
										<center><input type="checkbox" class="remove_all" name="remove_all[]" value="<?php echo $BookKeepingResult[$x]['id']; ?>"></center>
									</td>
								</tr>
							<?php }?>
						<?php }else{?>
				...
				<table cellspacing="0">
					<thead>
						<tr>
							...
							<th style="width:6%">Delete</th>
							<th><button class="btn-danger" onclick="showDeleteAllApprovedBookKeepingModal()">REMOVE ALL</button></th>
						</tr>
					</thead>
					<tbody>
						...
						<?php if(!empty($approveBookKeepingResult)){ ?>
							<?php for($x = 0; $x < count($approveBookKeepingResult); $x++){ ?>
								<tr>	
									...
									<td><?php echo $approveBookKeepingResult[$x]['paypal']; ?></td>
									<td>
																		<?php
									...
									<td>
										<?php
											echo date('Y-m-d H:i:s', $approveBookKeepingResult[$x]['date_updated']);
											
										?>
									</td>
									<td class="score-total"><?php echo $approveBookKeepingResult[$x]['test_score']; ?>%</td>
									<td>
										<button class="btn-default" onclick="showTestResultApprovedModal(<?php echo $approveBookKeepingResult[$x]['id']; ?>)">View Test</button>
										<div id="viewApproveTestModal<?php echo $approveBookKeepingResult[$x]['id']; ?>" class="modal">
											<div class="modal-content" style="width: 45%;">
												 <div class="modal-header">
													<span class="close" style="top: -10px;" onclick="closeModal(<?php echo "viewApproveTestModal" . (string)$approveBookKeepingResult[$x]['id']; ?>)">×</span>
													<h4 style="margin:0">Test Result: Multiple Choice</h4>
												 </div>
												 <div class="modal-body" style="padding-bottom:24px">
													<p style="margin-top:0;">Score: <span style="color:#18AACF;font-weight:bold;"><?php echo $approveBookKeepingResult[$x]['test_score']; ?>%</span></p>
													<div id="approved_test_result_<?php echo $approveBookKeepingResult[$x]['id']; ?>"></div>
												</div>
											</div>
										</div>
									</td>
									<td>
										<button class="btn-danger" onclick="showModal(<?php echo "deleteApprovedBookKeepingModal" . (string)$approveBookKeepingResult[$x]['id']; ?>)">REMOVE</button>
										<div id="deleteApprovedBookKeepingModal<?php echo $approveBookKeepingResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "deleteApprovedBookKeepingModal" . (string)$approveBookKeepingResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to remove?</h3>
												 </div>
												 <div class="modal-body">
													<form method="post" action="remove_BookKeeping_staff.php">
														<input type="hidden" name="id" value="<?php echo $approveBookKeepingResult[$x]['id']; ?>">
														<input type="hidden" name="check" value="2">
														<button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "deleteApprovedBookKeepingModal" . (string)$approveBookKeepingResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									<?php
										...
										$approvedDate = ($num_days * $per_day) + $approveBookKeepingResult[$x]['date_updated'];
										$approvedPlusDays = $approvedDate - $today_timestamp;
										
										if($approvedPlusDays <= 0){
									?>
										<center><input type="checkbox" class="remove_all_approved" name="remove_all_approved[]" value="<?php echo $approveBookKeepingResult[$x]['id']; ?>" checked></center>
									<?php }else{ ?> 
										<center><input type="checkbox" class="remove_all_approved" name="remove_all_approved[]" value="<?php echo $approveBookKeepingResult[$x]['id']; ?>"></center>
									<?php } ?>
...
<!--end email template-->
	...
	<div id="deleteAllModal" class="modal">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="post" action="remove_all_BookKeeping_staff.php">
	...
	<div id="deleteAllBookKeepingModal" class="modal">
		<div class="modal-content">
			<div class="modal-header">
				<span class="close" onclick="closeDeleteAllBookKeepingModal()">×</span>
				<h3 style="margin:0">Are you sure you want to remove all?</h3>
			</div>
			<div class="modal-body">
				<form method="post" action="remove_all_BookKeeping_staff.php">
					<input type="hidden" name="staff_ids">
					<button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
					<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeDeleteAllBookKeepingModal()">No</button>
				</form>
			</div>
		</div>
	</div>
	...
	<script>
		...
		function showDeleteAllApprovedBookKeepingModal() {
			
			var checkedVals = $('#approved .remove_all_approved:checkbox:checked').map(function() {
				return this.value;
			}).get();
			
			document.getElementById('deleteAllBookKeepingModal').style.display = "block";
			$('input[name=staff_ids]').val(checkedVals.join(","));
			
		}
		function showDeleteAllPendingBookKeepingModal() {
		...
		function closeDeleteAllBookKeepingModal() {
			document.getElementById('deleteAllBookKeepingModal').style.display = "none";
		}
		...
		function openTab(evt, cityName) {
		  ...
		  if(cityName == "approved" || cityName == "pending" || cityName == "mc_questions" || cityName == "email_template" || cityName == "BookKeeping"){
			 document.getElementById("BookKeeping").style.display = "block";
