...
$FileUpload = new FileUpload();
	...
	if($_POST['status'] == 0){
		...
		$getTranscriberEmailContent = mysqli_query($db, "SELECT * FROM ts_bookkeeping_approve_email_template WHERE type='1'");
		...
		$getProofreaderEmailContent = mysqli_query($db, "SELECT * FROM ts_bookkeeping_approve_email_template WHERE type='2'");
...
//		$bodyHtml = "";
		...
		$query = $crud->update('ts_bookkeeping_users', $data, "id", $_POST['id']);
		$result = $db->query($query);
