...
if(!isset($_GET['tab']) || (isset($_GET['tab']) && $_GET['tab'] == "pending")) {
	$getProofReadData = mysqli_query($db, "SELECT * FROM ts_bookkeeping_users WHERE status='0' ORDER BY date_created DESC");
	$proofReadResult = mysqli_fetch_all($getProofReadData, MYSQLI_ASSOC);
}
if(isset($_GET['tab']) && $_GET['tab'] == "approved") {
	$getProofReadApproved = mysqli_query($db, "SELECT * FROM ts_bookkeeping_users WHERE status='1' ORDER BY date_updated DESC");
...
<?php
														$q_id = $allQuestions[$q];
														$getSpecificQuestion = mysqli_query($db, "SELECT * FROM ts_bookkeeping_questions WHERE id='$q_id'");
													...
													?>
															...
															<ul style="margin-top:5px;display:inline;padding-left:5px;">
																<?php
																	...
																	$getTestResultChoices = mysqli_query($db, "SELECT * FROM ts_bookkeeping_question_choices WHERE question_id='$q_id'");
