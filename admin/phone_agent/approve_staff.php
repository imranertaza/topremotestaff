...
$crud = new Crud();
	...
	if($_POST['status'] == 0){
		...
		if($result){
			...
			$queryReject = $crud->add('ts_phoneagent_blacklist', $data);
		...
		$getTranscriberEmailContent = mysqli_query($db, "SELECT * FROM ts_phoneagent_approve_email_template WHERE type='1'");
		...
		$getProofreaderEmailContent = mysqli_query($db, "SELECT * FROM ts_phoneagent_approve_email_template WHERE type='2'");
		...
		$query = $crud->update('ts_phoneagent_users', $data, "id", $_POST['id']);
		$result = $db->query($query);
