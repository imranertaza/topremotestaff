...
if (!isset($_GET['tab']) || (isset($_GET['tab']) && $_GET['tab'] == "pending")) {
    $getphoneagentData = mysqli_query($db, "SELECT * FROM ts_phoneagent_users WHERE status='0' ORDER BY date_created DESC");
    $phoneagentResult = mysqli_fetch_all($getphoneagentData, MYSQLI_ASSOC);
}
if (isset($_GET['tab']) && $_GET['tab'] == "approved") {
    $getphoneagentApproved = mysqli_query($db, "SELECT * FROM ts_phoneagent_users WHERE status='1' ORDER BY date_updated DESC");
    $approvephoneagentResult = mysqli_fetch_all($getphoneagentApproved, MYSQLI_ASSOC);
}
if (isset($_GET['tab']) && $_GET['tab'] == "mc_questions") {
    $getQuestions = mysqli_query($db, "SELECT * FROM ts_phoneagent_questions WHERE status='1' ORDER BY date_created DESC");
    $questionResult = mysqli_fetch_all($getQuestions, MYSQLI_ASSOC);
}
if (isset($_GET['tab']) && $_GET['tab'] == "email_template") {
    $getApproveEmailTemplate = mysqli_query($db, "SELECT * FROM ts_phoneagent_approve_email_template");
    $approveEmailTemplate = mysqli_fetch_all($getApproveEmailTemplate, MYSQLI_ASSOC);

    $getFollowupEmailTemplate = mysqli_query($db, "SELECT * FROM ts_phoneagent_followup_email_template");
...
<h2>Phone Agent </h2>
<div id="phoneagent" class="tabcontent p-1" style="display: block;">
    <div class="tab">
        <?php
        if (!empty($_GET)) {
        ...
        <div id="pending" class="tabcontent p-1 show">
            <table cellspacing="0">
                ...
                <tr>
                    ...
                    <th>
                        <button class="btn-danger" onclick="showDeleteAllPendingphoneagentModal()">REMOVE ALL
                        </button>
                    </th>
                </tr>
                </thead>
                ...
                <?php if (!isset($_GET['tab']) || $_GET['tab'] == "pending") { ?>
                    <?php if (mysqli_num_rows($getphoneagentData) > 0) { ?>
                        <?php for ($x = 0; $x < count($phoneagentResult); $x++) { ?>
                            <tr>
                                <td><?php echo urldecode($phoneagentResult[$x]['fullname']); ?></td>
                                <td><?php echo $phoneagentResult[$x]['email']; ?></td>
                                <td><?php echo $phoneagentResult[$x]['phone']; ?></td>
                                <td><?php echo $phoneagentResult[$x]['skype']; ?></td>
                                <td><?php echo $phoneagentResult[$x]['source']; ?></td>
                                <td>
                                    <button class="showCV"
                                            onclick="showCVModal('<?php print$phoneagentResult[$x]['cv']; ?>')"
                                            id="myBtn">View CV
                                    </button>
                                </td>
                                <td>
                                    <button class="btn btn-default"
                                            onclick="showVoiceModal('<?php print $phoneagentResult[$x]['voice_record']; ?>')">
                                        View Voice Record
                                    </button>
                                </td>
                                <td class="typing-total">
                                    Accuracy: <?php echo $phoneagentResult[$x]['typing_test_accuracy']; ?><br>
                                    Speed: <?php echo $phoneagentResult[$x]['typing_test_speed']; ?>
                               </td>
                                <td class="score-total"><?php echo $phoneagentResult[$x]['test_score']; ?>%</td>
                                <td>
                                    <button class="btn-default"
                                            onclick="showTestResultModal(<?php echo $phoneagentResult[$x]['id']; ?>)">
                                        View Test
                                    </button>
                                    <div id="viewTestResultModal<?php echo $phoneagentResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content" style="width: 45%;">
                                            <div class="modal-header">
                                                <span class="close" style="top: -10px;"
                                                      onclick="closeModal(<?php echo "viewTestResultModal" . (string)$phoneagentResult[$x]['id']; ?>)">×</span>
                                            ...
                                            <div class="modal-body" style="padding-bottom:24px">
                                                <p style="margin-top:0;">Score: <span
                                                            style="color:#18AACF;font-weight:bold;"><?php echo $phoneagentResult[$x]['test_score']; ?>%</span>
                                                </p>
                                                <div id="test_result_<?php echo $phoneagentResult[$x]['id']; ?>"></div>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <button class="btn-success"
                                            onclick="showModal(<?php echo "approvephoneagentmodal" . (string)$phoneagentResult[$x]['id']; ?>)">
                                        APPROVE
                                    </button>
                                    <button class="btn-danger"
                                            onclick="showModal(<?php echo "declinephoneagentmodal" . (string)$phoneagentResult[$x]['id']; ?>)">
                                        DECLINE
                                    </button>
                                    <div id="approvephoneagentmodal<?php echo $phoneagentResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <span class="close"
                                                      onclick="closeModal(<?php echo "approvephoneagentmodal" . (string)$phoneagentResult[$x]['id']; ?>)">×</span>
                                                <h3 style="margin:0">Are you sure you want to approve?</h3>
                                            </div>
                                            <div class="modal-body">
                                                <form method="post" action="approve_phoneagent_staff.php"
                                                      style="display:inline">
                                                    <input type="hidden" name="id"
                                                           value="<?php echo $phoneagentResult[$x]['id']; ?>">
                                                    <input type="hidden" name="email"
                                                           value="<?php echo $phoneagentResult[$x]['email']; ?>">
                                                    <input type="hidden" name="fullname"
                                                           value="<?php echo $phoneagentResult[$x]['fullname']; ?>">
                                                ...
                                                <form method="post" action="approve_phoneagent_staff.php"
                                                    ...
                                                    <input type="hidden" name="status" value="1">
                                                    <button type="submit" class="btn-success" style="font-size:12px;">
                                                        Yes, as PHONE AGENT QC
                                                    </button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                                    <div id="declinephoneagentmodal<?php echo $phoneagentResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <span class="close"
                                                      onclick="closeModal(<?php echo "declinephoneagentmodal" . (string)$phoneagentResult[$x]['id']; ?>)">×</span>
                                            ...
                                            <div class="modal-body">
                                                <form method="post" action="approve_phoneagent_staff.php">
                                                    ...
                                                    <input type="hidden" name="skype"
                                                           value="<?php echo $phoneagentResult[$x]['skype']; ?>">
                                                    <input type="hidden" name="email"
                                                           value="<?php echo $phoneagentResult[$x]['email']; ?>">
                                                    <input type="hidden" name="paypal"
                                                           value="<?php echo $phoneagentResult[$x]['paypal']; ?>">
                                                    <input type="hidden" name="fullname"
                                                           value="<?php echo $phoneagentResult[$x]['fullname']; ?>">
                                                    ...
                                                    <button type="button" class="btn-default" style="font-size:1rem;"
                                                            onclick="closeModal(<?php echo "declinephoneagentmodal" . (string)$phoneagentResult[$x]['id']; ?>)">
                                                        No
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                <td>
                                    <button class="btn-danger"
                                            onclick="showModal(<?php echo "deletePendingphoneagentModal" . (string)$phoneagentResult[$x]['id']; ?>)">
                                        REMOVE
                                    </button>
                                    <div id="deletePendingphoneagentModal<?php echo $phoneagentResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <span class="close"
                                                      onclick="closeModal(<?php echo "deletePendingphoneagentModal" . (string)$phoneagentResult[$x]['id']; ?>)">×</span>
                                                <h3 style="margin:0">Are you sure you want to remove?</h3>
                                            </div>
                                            <div class="modal-body">
                                                <form method="post" action="remove_phoneagent_staff.php">
                                                    <input type="hidden" name="id"
                                                           value="<?php echo $phoneagentResult[$x]['id']; ?>">
                                                    ...
                                                    <button type="button" class="btn-default" style="font-size:1rem;"
                                                            onclick="closeModal(<?php echo "deletePendingphoneagentModal" . (string)$phoneagentResult[$x]['id']; ?>)">
                                                        No
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                <td>
                                    <center><input type="checkbox" class="remove_all" name="remove_all[]"
                                                   value="<?php echo $phoneagentResult[$x]['id']; ?>"></center>
                                </td>
                            </tr>
                        <?php } ?>
                    <?php } else { ?>
        ...
        <div id="approved" class="tabcontent p-1 show">
            <table cellspacing="0">
                ...
                <tr>
                    ...
                    <th style="width:6%">Delete</th>
                    <th>
                        <button class="btn-danger" onclick="showDeleteAllApprovedphoneagentModal()">REMOVE ALL
                        </button>
                    </th>
                </tr>
                </thead>
                ...
                <?php if (!isset($_GET['tab']) || $_GET['tab'] == "approved") { ?>
                    <?php if (!empty($approvephoneagentResult)) { ?>
                        <?php for ($x = 0; $x < count($approvephoneagentResult); $x++) { ?>
                            <tr>
                                <td><?php echo urldecode($approvephoneagentResult[$x]['fullname']); ?></td>
                                <td><?php echo $approvephoneagentResult[$x]['email']; ?></td>
                                <td><?php echo $approvephoneagentResult[$x]['phone']; ?></td>
                                <td><?php echo $approvephoneagentResult[$x]['skype']; ?></td>
                                <td><?php echo $approvephoneagentResult[$x]['paypal']; ?></td>
                                <td>
                                    <?php
                                    if ($approvephoneagentResult[$x]['account_type'] == 1) {
                                        echo "PHONE AGENT";
                                    } else {
                                        echo "PHONE AGENT QC";
                                    }
                                ...
                                <td>
                                    <?php
                                    echo date('Y-m-d H:i:s', $approvephoneagentResult[$x]['date_updated']);

                                    ?>
                                </td>
                                <td class="score-total"><?php echo $approvephoneagentResult[$x]['test_score']; ?>
                                    %
                                </td>
                                <td>
                                    <button class="btn-default"
                                            onclick="showTestResultApprovedModal(<?php echo $approvephoneagentResult[$x]['id']; ?>)">
                                        View Test
                                    </button>
                                    <div id="viewApproveTestModal<?php echo $approvephoneagentResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content" style="width: 45%;">
                                            <div class="modal-header">
                                                <span class="close" style="top: -10px;"
                                                      onclick="closeModal(<?php echo "viewApproveTestModal" . (string)$approvephoneagentResult[$x]['id']; ?>)">×</span>
                                                <h4 style="margin:0">Test Result: Multiple Choice</h4>
                                            </div>
                                            <div class="modal-body" style="padding-bottom:24px">
                                                <p style="margin-top:0;">Score: <span
                                                            style="color:#18AACF;font-weight:bold;"><?php echo $approvephoneagentResult[$x]['test_score']; ?>%</span>
                                                </p>
                                                <div id="approved_test_result_<?php echo $approvephoneagentResult[$x]['id']; ?>"></div>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <button class="btn-danger"
                                            onclick="showModal(<?php echo "deleteApprovedphoneagentModal" . (string)$approvephoneagentResult[$x]['id']; ?>)">
                                        REMOVE
                                    </button>
                                    <div id="deleteApprovedphoneagentModal<?php echo $approvephoneagentResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <span class="close"
                                                      onclick="closeModal(<?php echo "deleteApprovedphoneagentModal" . (string)$approvephoneagentResult[$x]['id']; ?>)">×</span>
                                                <h3 style="margin:0">Are you sure you want to remove?</h3>
                                            </div>
                                            <div class="modal-body">
                                                <form method="post" action="remove_phoneagent_staff.php">
                                                    <input type="hidden" name="id"
                                                           value="<?php echo $approvephoneagentResult[$x]['id']; ?>">
                                                    <input type="hidden" name="check" value="2">
                                                    <button type="submit" class="btn-danger" style="font-size:1rem;">
                                                        Yes
                                                    </button>
                                                    <button type="button" class="btn-default" style="font-size:1rem;"
                                                            onclick="closeModal(<?php echo "deleteApprovedphoneagentModal" . (string)$approvephoneagentResult[$x]['id']; ?>)">
                                                        No
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                <td>
                                    ...
                                    $approvedDate = ($num_days * $per_day) + $approvephoneagentResult[$x]['date_updated'];
            ...
            <?php if (mysqli_num_rows($getQuestions) > 0) { ?>
                <?php for ($q = 0; $q < count($questionResult); $q++) { ?>
                    ...
                    <ul style="margin-top:5px;display:inline;padding-left:5px;">
                        ...
                        $getChoices = mysqli_query($db, "SELECT * FROM ts_phoneagent_question_choices WHERE question_id='$id'");
            ...
            <table style="border:0;">
                ...
                <tr>
                    <td style="width:40%;vertical-align: top;border:0;">
                        <small>APPROVE EMAIL</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: PHONE AGENT </strong>
                        <br/>
                        <form action="save_phoneagent_approve_email_template.php" method="post">
                        ...
                        <br/>
                        <small>APPROVE EMAIL</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: PHONE AGENT QC</strong>
                        <br/>
                        <form action="save_phoneagent_approve_email_template.php" method="post">
                        ...
                        <br/>
                        <small>AUTO CREATE ACCOUNT</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: PHONE AGENT </strong>
                        <br/>
                        <form action="save_phoneagent_approve_email_template.php" method="post">
                        ...
                        <br/>
                        <small>AUTO CREATE ACCOUNT</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: PHONE AGENT QC</strong>
                        <br/>
                        <form action="save_phoneagent_approve_email_template.php" method="post">
                    ...
                    <td style="width:40%;vertical-align: top;border:0" class="follow-up-email-container">
                        <?php for ($d = 0; $d < count($followupEmailTemplate); $d++) { ?>
                            <div class="follow-up-emails">
                                ...
                                <br/>
                                <form action="save_phoneagent_followup_email_template.php" method="post">
                                    <textarea name="content" rows="15"
...
<div id="deleteAllModal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form method="post" action="remove_all_phoneagent_staff.php">
...
<div id="myModal" class="modal">
    ...
    <div class="modal-content">
        ...
        <embed id="embedPDF"
               src="<?php //echo "https://topremotestaff.us-east-1.linodeobjects.com/cv/".$phoneagentResult[$x]['cv']; ?>"
...
<div id="deleteAllphoneagentModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <span class="close" onclick="closeDeleteAllphoneagentModal()">×</span>
            <h3 style="margin:0">Are you sure you want to remove all?</h3>
        </div>
        <div class="modal-body">
            <form method="post" action="remove_all_phoneagent_staff.php">
                ...
                <button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
                <button type="button" class="btn-default" style="font-size:1rem;"
                        onclick="closeDeleteAllphoneagentModal()">No
                </button>
            </form>
        </div>
    </div>
...
<script>
    ...
    function showDeleteAllApprovedphoneagentModal() {

        var checkedVals = $('#approved .remove_all_approved:checkbox:checked').map(function () {
            return this.value;
        }).get();

        document.getElementById('deleteAllphoneagentModal').style.display = "block";
        $('input[name=staff_ids]').val(checkedVals.join(","));

    }

    function showDeleteAllPendingphoneagentModal() {
    ...
    function closeDeleteAllphoneagentModal() {
        document.getElementById('deleteAllphoneagentModal').style.display = "none";
    }
    ...
    function openTab(evt, cityName) {
        ...
        if (cityName == "approved" || cityName == "pending" || cityName == "mc_questions" || cityName == "email_template" || cityName == "phoneagent") {
            document.getElementById("phoneagent").style.display = "block";
