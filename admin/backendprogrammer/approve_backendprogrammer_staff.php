...
$FileUpload = new FileUpload();
	...
	if($_POST['status'] == 0){
		...
		$selectQuery = $crud->getSelectedData('cv', 'ts_backendprogrammer_users', "id", $_POST['id']);
		...
		$query = $crud->delete('ts_backendprogrammer_users', "id", $_POST['id']);
		...
		if($result){
			...
			$queryReject = $crud->add('ts_backendprogrammer_blacklist', $data);
		...
		$getTranscriberEmailContent = mysqli_query($db, "SELECT * FROM ts_backendprogrammer_approve_email_template WHERE type='1'");
		...
		$getProofreaderEmailContent = mysqli_query($db, "SELECT * FROM ts_backendprogrammer_approve_email_template WHERE type='2'");
...
//		$bodyHtml = "";
		...
		$query = $crud->update('ts_backendprogrammer_users', $data, "id", $_POST['id']);
		$result = $db->query($query);
