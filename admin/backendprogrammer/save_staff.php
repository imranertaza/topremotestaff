...
$date = new DateTime();
	...
	if(!empty($_POST)){
		...
		for($x = 0 ; $x < count($question); $x++){
			...
			$getAnswer = mysqli_query($db, "SELECT correct FROM ts_backendprogrammer_question_choices WHERE question_id='$question_id'");
		...
		$score = round($score,2);
		$checkEmail = $db->query($crud->getOrData('ts_backendprogrammer_users', array("email"), array($_POST['email'])));
		$checkBlackList = $db->query($crud->getOrData('ts_backendprogrammer_blacklist', array("email"), array($_POST['email'])));
		...
		if ($checkEmail->num_rows > 0) {
			...
			if($checkBlackList->num_rows > 0) {
				...
				$query = $crud->add('ts_backendprogrammer_users', $data);
				...
				if($result){
					...
					$answer_data = array(
						'backendprogrammerg_user_id' => $insert_id,
						'question_details'  => json_encode($question),
						'answer_details'    => json_encode($answers),
						//'date_created'      => date("Y-m-d H:i:s")
						'date_created' => date("Y-m-d H:i:s" , $date->getTimestamp())
					);
					$db->query($crud->add('ts_backendprogrammer_answers', $answer_data));
