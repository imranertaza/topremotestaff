...
$FileUpload = new FileUpload();
	...
	if($_POST['status'] == 0){
		...
		$selectQuery = $crud->getSelectedData('cv', 'ts_livechat_users', "id", $_POST['id']);
		...
		// Deleting CV from the S3 storage server (Start)
		$selectQuery = $crud->getSelectedData('voice_record', 'ts_livechat_users', "id", $_POST['id']);
		$getResult = $db->query($selectQuery);
		...
		$query = $crud->delete('ts_livechat_users', "id", $_POST['id']);
		...
		if($result){
			...
			$queryReject = $crud->add('ts_livechat_blacklist', $data);
		...
		$getTranscriberEmailContent = mysqli_query($db, "SELECT * FROM ts_livechat_approve_email_template WHERE type='1'");
		...
		$getProofreaderEmailContent = mysqli_query($db, "SELECT * FROM ts_livechat_approve_email_template WHERE type='2'");
...
//		$bodyHtml = "";
		...
		$query = $crud->update('ts_livechat_users', $data, "id", $_POST['id']);
		$result = $db->query($query);
