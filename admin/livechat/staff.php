...
if (!isset($_GET['tab']) || (isset($_GET['tab']) && $_GET['tab'] == "pending")) {
    $getlivechatData = mysqli_query($db, "SELECT * FROM ts_livechat_users WHERE status='0' ORDER BY date_created DESC");
    $livechatResult = mysqli_fetch_all($getlivechatData, MYSQLI_ASSOC);
}
if (isset($_GET['tab']) && $_GET['tab'] == "approved") {
    $getlivechatApproved = mysqli_query($db, "SELECT * FROM ts_livechat_users WHERE status='1' ORDER BY date_updated DESC");
    $approvelivechatResult = mysqli_fetch_all($getlivechatApproved, MYSQLI_ASSOC);
}
if (isset($_GET['tab']) && $_GET['tab'] == "mc_questions") {
    $getQuestions = mysqli_query($db, "SELECT * FROM ts_livechat_questions WHERE status='1' ORDER BY date_created DESC");
    $questionResult = mysqli_fetch_all($getQuestions, MYSQLI_ASSOC);
}
if (isset($_GET['tab']) && $_GET['tab'] == "email_template") {
    $getApproveEmailTemplate = mysqli_query($db, "SELECT * FROM ts_livechat_approve_email_template");
    $approveEmailTemplate = mysqli_fetch_all($getApproveEmailTemplate, MYSQLI_ASSOC);

    $getFollowupEmailTemplate = mysqli_query($db, "SELECT * FROM ts_livechat_followup_email_template");
...
<h2>Live Chat</h2>
<div id="livechat" class="tabcontent p-1" style="display: block;">
    <div class="tab">
        <?php
        if (!empty($_GET)) {
        ...
        <div id="pending" class="tabcontent p-1 show">
            <table cellspacing="0">
                ...
                <tr>
                    ...
                    <th>
                        <button class="btn-danger" onclick="showDeleteAllPendinglivechatModal()">REMOVE ALL
                        </button>
                    </th>
                </tr>
                </thead>
                ...
                <?php if (!isset($_GET['tab']) || $_GET['tab'] == "pending") { ?>
                    <?php if (mysqli_num_rows($getlivechatData) > 0) { ?>
                        <?php for ($x = 0; $x < count($livechatResult); $x++) { ?>
                            <tr>
                                <td><?php echo urldecode($livechatResult[$x]['fullname']); ?></td>
                                <td><?php echo $livechatResult[$x]['email']; ?></td>
                                <td><?php echo $livechatResult[$x]['phone']; ?></td>
                                <td><?php echo $livechatResult[$x]['skype']; ?></td>
                                <td><?php echo $livechatResult[$x]['source']; ?></td>
                                <td>
                                    <button class="showCV"
                                            onclick="showCVModal('<?php print$livechatResult[$x]['cv']; ?>')"
                                            id="myBtn">View CV
                                    </button>
                                </td>

                                <td class="score-total"><?php echo $livechatResult[$x]['test_score']; ?>%</td>
                                <td>
                                    <button class="btn-default"
                                            onclick="showTestResultModal(<?php echo $livechatResult[$x]['id']; ?>)">
                                        View Test
                                    </button>
                                    <div id="viewTestResultModal<?php echo $livechatResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content" style="width: 45%;">
                                            <div class="modal-header">
                                                <span class="close" style="top: -10px;"
                                                      onclick="closeModal(<?php echo "viewTestResultModal" . (string)$livechatResult[$x]['id']; ?>)">×</span>
                                            ...
                                            <div class="modal-body" style="padding-bottom:24px">
                                                <p style="margin-top:0;">Score: <span
                                                            style="color:#18AACF;font-weight:bold;"><?php echo $livechatResult[$x]['test_score']; ?>%</span>
                                                </p>
                                                <div id="test_result_<?php echo $livechatResult[$x]['id']; ?>"></div>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <button class="btn-success"
                                            onclick="showModal(<?php echo "approvelivechatmodal" . (string)$livechatResult[$x]['id']; ?>)">
                                        APPROVE
                                    </button>
                                    <button class="btn-danger"
                                            onclick="showModal(<?php echo "declinelivechatmodal" . (string)$livechatResult[$x]['id']; ?>)">
                                        DECLINE
                                    </button>
                                    <div id="approvelivechatmodal<?php echo $livechatResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <span class="close"
                                                      onclick="closeModal(<?php echo "approvelivechatmodal" . (string)$livechatResult[$x]['id']; ?>)">×</span>
                                                <h3 style="margin:0">Are you sure you want to approve?</h3>
                                            </div>
                                            <div class="modal-body">
                                                <form method="post" action="approve_livechat_staff.php"
                                                      style="display:inline">
                                                    <input type="hidden" name="id"
                                                           value="<?php echo $livechatResult[$x]['id']; ?>">
                                                    <input type="hidden" name="email"
                                                           value="<?php echo $livechatResult[$x]['email']; ?>">
                                                    <input type="hidden" name="fullname"
                                                           value="<?php echo $livechatResult[$x]['fullname']; ?>">
                                    ...
                                    <div id="declinelivechatmodal<?php echo $livechatResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <span class="close"
                                                      onclick="closeModal(<?php echo "declinelivechatmodal" . (string)$livechatResult[$x]['id']; ?>)">×</span>
                                            ...
                                            <div class="modal-body">
                                                <form method="post" action="approve_livechat_staff.php">
                                                    ...
                                                    <input type="hidden" name="skype"
                                                           value="<?php echo $livechatResult[$x]['skype']; ?>">
                                                    <input type="hidden" name="email"
                                                           value="<?php echo $livechatResult[$x]['email']; ?>">
                                                    <input type="hidden" name="paypal"
                                                           value="<?php echo $livechatResult[$x]['paypal']; ?>">
                                                    <input type="hidden" name="fullname"
                                                           value="<?php echo $livechatResult[$x]['fullname']; ?>">
                                                    ...
                                                    <button type="button" class="btn-default" style="font-size:1rem;"
                                                            onclick="closeModal(<?php echo "declinelivechatmodal" . (string)$livechatResult[$x]['id']; ?>)">
                                                        No
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                <td>
                                    <button class="btn-danger"
                                            onclick="showModal(<?php echo "deletePendinglivechatModal" . (string)$livechatResult[$x]['id']; ?>)">
                                        REMOVE
                                    </button>
                                    <div id="deletePendinglivechatModal<?php echo $livechatResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <span class="close"
                                                      onclick="closeModal(<?php echo "deletePendinglivechatModal" . (string)$livechatResult[$x]['id']; ?>)">×</span>
                                                <h3 style="margin:0">Are you sure you want to remove?</h3>
                                            </div>
                                            <div class="modal-body">
                                                <form method="post" action="remove_livechat_staff.php">
                                                    <input type="hidden" name="id"
                                                           value="<?php echo $livechatResult[$x]['id']; ?>">
                                                    ...
                                                    <button type="button" class="btn-default" style="font-size:1rem;"
                                                            onclick="closeModal(<?php echo "deletePendinglivechatModal" . (string)$livechatResult[$x]['id']; ?>)">
                                                        No
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                <td>
                                    <center><input type="checkbox" class="remove_all" name="remove_all[]"
                                                   value="<?php echo $livechatResult[$x]['id']; ?>"></center>
                                </td>
                            </tr>
                        <?php } ?>
                    <?php } else { ?>
        ...
        <div id="approved" class="tabcontent p-1 show">
            <table cellspacing="0">
                ...
                <tr>
                    ...
                    <th style="width:6%">Delete</th>
                    <th>
                        <button class="btn-danger" onclick="showDeleteAllApprovedlivechatModal()">REMOVE ALL
                        </button>
                    </th>
                </tr>
                </thead>
                ...
                <?php if (!isset($_GET['tab']) || $_GET['tab'] == "approved") { ?>
                    <?php if (!empty($approvelivechatResult)) { ?>
                        <?php for ($x = 0; $x < count($approvelivechatResult); $x++) { ?>
                            <tr>
                                <td><?php echo urldecode($approvelivechatResult[$x]['fullname']); ?></td>
                                <td><?php echo $approvelivechatResult[$x]['email']; ?></td>
                                <td><?php echo $approvelivechatResult[$x]['phone']; ?></td>
                                <td><?php echo $approvelivechatResult[$x]['skype']; ?></td>
                                <td><?php echo $approvelivechatResult[$x]['paypal']; ?></td>
                                <td>
                                    <?php
                                    if ($approvelivechatResult[$x]['account_type'] == 1) {
                                        echo "Live Chat";
                                    } else {
                                        echo "Live Chat QC";
                                    }
                                ...
                                <td>
                                    <?php
                                    echo date('Y-m-d H:i:s', $approvelivechatResult[$x]['date_updated']);

                                    ?>
                                </td>
                                <td class="score-total"><?php echo $approvelivechatResult[$x]['test_score']; ?>
                                    %
                                </td>
                                <td>
                                    <button class="btn-default"
                                            onclick="showTestResultApprovedModal(<?php echo $approvelivechatResult[$x]['id']; ?>)">
                                        View Test
                                    </button>
                                    <div id="viewApproveTestModal<?php echo $approvelivechatResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content" style="width: 45%;">
                                            <div class="modal-header">
                                                <span class="close" style="top: -10px;"
                                                      onclick="closeModal(<?php echo "viewApproveTestModal" . (string)$approvelivechatResult[$x]['id']; ?>)">×</span>
                                                <h4 style="margin:0">Test Result: Multiple Choice</h4>
                                            </div>
                                            <div class="modal-body" style="padding-bottom:24px">
                                                <p style="margin-top:0;">Score: <span
                                                            style="color:#18AACF;font-weight:bold;"><?php echo $approvelivechatResult[$x]['test_score']; ?>%</span>
                                                </p>
                                                <div id="approved_test_result_<?php echo $approvelivechatResult[$x]['id']; ?>"></div>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <button class="btn-danger"
                                            onclick="showModal(<?php echo "deleteApprovedlivechatModal" . (string)$approvelivechatResult[$x]['id']; ?>)">
                                        REMOVE
                                    </button>
                                    <div id="deleteApprovedlivechatModal<?php echo $approvelivechatResult[$x]['id']; ?>"
                                         class="modal">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <span class="close"
                                                      onclick="closeModal(<?php echo "deleteApprovedlivechatModal" . (string)$approvelivechatResult[$x]['id']; ?>)">×</span>
                                                <h3 style="margin:0">Are you sure you want to remove?</h3>
                                            </div>
                                            <div class="modal-body">
                                                <form method="post" action="remove_livechat_staff.php">
                                                    <input type="hidden" name="id"
                                                           value="<?php echo $approvelivechatResult[$x]['id']; ?>">
                                                    <input type="hidden" name="check" value="2">
                                                    <button type="submit" class="btn-danger" style="font-size:1rem;">
                                                        Yes
                                                    </button>
                                                    <button type="button" class="btn-default" style="font-size:1rem;"
                                                            onclick="closeModal(<?php echo "deleteApprovedlivechatModal" . (string)$approvelivechatResult[$x]['id']; ?>)">
                                                        No
                                                    </button>
                                                </form>
                                            </div>
                                ...
                                <td>
                                    ...
                                    $approvedDate = ($num_days * $per_day) + $approvelivechatResult[$x]['date_updated'];
            ...
            <table style="border:0;">
                ...
                <tr>
                    <td style="width:40%;vertical-align: top;border:0;">
                        <small>APPROVE EMAIL</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: LIVE CHAT</strong>
                        <br/>
                        <form action="save_livechat_approve_email_template.php" method="post">
                        ...
                        <br/>
                        <small>APPROVE EMAIL</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: LIVE CHAT
                            QC</strong>
                        <br/>
                        <form action="save_livechat_approve_email_template.php" method="post">
                        ...
                        <br/>
                        <small>AUTO CREATE ACCOUNT</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: LIVE CHAT</strong>
                        <br/>
                        <form action="save_livechat_approve_email_template.php" method="post">
                        ...
                        <br/>
                        <small>AUTO CREATE ACCOUNT</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: LIVE CHAT QC</strong>
                        <br/>
                        <form action="save_livechat_approve_email_template.php" method="post">
                    ...
                    <td style="width:40%;vertical-align: top;border:0" class="follow-up-email-container">
                        <?php for ($d = 0; $d < count($followupEmailTemplate); $d++) { ?>
                            <div class="follow-up-emails">
                                ...
                                <br/>
                                <form action="save_livechat_followup_email_template.php" method="post">
                                    <textarea name="content" rows="15"
...
<div id="deleteAllModal" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form method="post" action="remove_all_livechat_staff.php">
...
<div id="myModal" class="modal">
    ...
    <div class="modal-content">
        ...
        <embed id="embedPDF"
               src="<?php //echo "https://topremotestaff.us-east-1.linodeobjects.com/cv/".$livechatResult[$x]['cv']; ?>"
...
<div id="deleteAlllivechatModal" class="modal">
    <div class="modal-content">
        <div class="modal-header">
            <span class="close" onclick="closeDeleteAlllivechatModal()">×</span>
            <h3 style="margin:0">Are you sure you want to remove all?</h3>
        </div>
        <div class="modal-body">
            <form method="post" action="remove_all_livechat_staff.php">
                ...
                <button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
                <button type="button" class="btn-default" style="font-size:1rem;"
                        onclick="closeDeleteAlllivechatModal()">No
                </button>
            </form>
        </div>
    </div>
...
<script>
    ...
    function showDeleteAllApprovedlivechatModal() {

        var checkedVals = $('#approved .remove_all_approved:checkbox:checked').map(function () {
            return this.value;
        }).get();

        document.getElementById('deleteAlllivechatModal').style.display = "block";
        $('input[name=staff_ids]').val(checkedVals.join(","));

    }

    function showDeleteAllPendinglivechatModal() {
    ...
    function closeDeleteAlllivechatModal() {
        document.getElementById('deleteAlllivechatModal').style.display = "none";
    }
    ...
    function openTab(evt, cityName) {
        ...
        if (cityName == "approved" || cityName == "pending" || cityName == "mc_questions" || cityName == "email_template" || cityName == "livechat") {
