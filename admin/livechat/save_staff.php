...
$date = new DateTime();
	...
	if(!empty($_POST)){
		...
		$score = round($score,2);
		$checkEmail = $db->query($crud->getOrData('ts_livechat_users', array("email"), array($_POST['email'])));
		$checkBlackList = $db->query($crud->getOrData('ts_livechat_blacklist', array("email"), array($_POST['email'])));
