...
$getData = mysqli_query($db, "SELECT * FROM ts_livechat_users WHERE status='0' ORDER BY date_created DESC");
$result = mysqli_fetch_all($getData, MYSQLI_ASSOC);

$getApproved = mysqli_query($db, "SELECT * FROM ts_livechat_users WHERE status='1' ORDER BY date_updated DESC");
$approveResult = mysqli_fetch_all($getApproved, MYSQLI_ASSOC);	

$getTest = mysqli_query($db, "SELECT * FROM ts_livechat_test ORDER BY id DESC");
$testResult = mysqli_fetch_all($getTest, MYSQLI_ASSOC);

$getTestData = mysqli_query($db, "SELECT * FROM ts_livechat_test WHERE id='2'");
