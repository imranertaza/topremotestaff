...
$FileUpload = new FileUpload();
    ...
    foreach($allIds as $id) {
        $selectQuery = $crud->getSelectedData('cv', 'ts_livechat_users', "id", $id);
    ...
    // Deleting CV from the S3 storage server (Start)
    foreach($allIds as $id) {
        $selectQuery = $crud->getSelectedData('voice_record', 'ts_livechat_users', "id", $id);
        $getResult = $db->query($selectQuery);
    ...
    $queryQu = $crud->deleteAll('ts_livechat_answers', "livechat_user_id", $_POST['staff_ids']);
    $db->query($queryQu);
    //deleting question answer

	$query = $crud->deleteAll('ts_livechat_users', "id", $_POST['staff_ids']);
