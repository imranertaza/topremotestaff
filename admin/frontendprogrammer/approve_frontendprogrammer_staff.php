...
$FileUpload = new FileUpload();
	...
	if($_POST['status'] == 0){
		...
		$selectQuery = $crud->getSelectedData('cv', 'ts_frontendprogrammer_users', "id", $_POST['id']);
		...
		$query = $crud->delete('ts_frontendprogrammer_users', "id", $_POST['id']);
		...
		if($result){
			...
			$queryReject = $crud->add('ts_frontendprogrammer_blacklist', $data);
		...
		$getTranscriberEmailContent = mysqli_query($db, "SELECT * FROM ts_frontendprogrammer_approve_email_template WHERE type='1'");
		...
		$getProofreaderEmailContent = mysqli_query($db, "SELECT * FROM ts_frontendprogrammer_approve_email_template WHERE type='2'");
...
//		$bodyHtml = "";
		...
		$query = $crud->update('ts_frontendprogrammer_users', $data, "id", $_POST['id']);
		$result = $db->query($query);
