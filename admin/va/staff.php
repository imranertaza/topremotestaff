				...
				<table cellspacing="0">
					<thead>
						<tr>
							...
							<th><button class="btn-danger" onclick="showDeleteAllPendingvirtualassistantModal()">REMOVE ALL</button></th>
						</tr>
					</thead>
					<tbody>
						...
						<?php if(mysqli_num_rows($getvirtualassistantData) > 0){ ?>
							<?php for($x = 0; $x < count($virtualassistantResult); $x++){ ?>
								<tr>	
									...
									<td>
                                        <button class="showCV" onclick="showCVModal('<?php print$virtualassistantResult[$x]['cv']; ?>')"  id="myBtn">View CV</button>
                                    </td>
                                    <td>
                                        <button class="btn btn-default" onclick="showVoiceModal('<?php print $virtualassistantResult[$x]['voice_record']; ?>')"  >View Voice Record</button>
                                    </td>
									<td class="score-total"><?php echo $virtualassistantResult[$x]['test_score']; ?>%</td>
									<td>
										<button class="btn-default" onclick="showTestResultModal(<?php echo $virtualassistantResult[$x]['id']; ?>)">View Test</button>
										<div id="viewTestResultModal<?php echo $virtualassistantResult[$x]['id']; ?>" class="modal">
											<div class="modal-content" style="width: 45%;">
												 <div class="modal-header">
													<span class="close" style="top: -10px;" onclick="closeModal(<?php echo "viewTestResultModal" . (string)$virtualassistantResult[$x]['id']; ?>)">×</span>
													<h4 style="margin:0">Test Result: Multiple Choice</h4>
												 </div>
												 <div class="modal-body" style="padding-bottom:24px">
													<p style="margin-top:0;">Score: <span style="color:#18AACF;font-weight:bold;"><?php echo $virtualassistantResult[$x]['test_score']; ?>%</span></p>
									...
									<td>
										<button class="btn-success" onclick="showModal(<?php echo "approvevirtualassistantmodal" . (string)$virtualassistantResult[$x]['id']; ?>)">APPROVE</button>
										<button class="btn-danger" onclick="showModal(<?php echo "declinevirtualassistantmodal" . (string)$virtualassistantResult[$x]['id']; ?>)">DECLINE</button>
										<div id="approvevirtualassistantmodal<?php echo $virtualassistantResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "approvevirtualassistantmodal" . (string)$virtualassistantResult[$x]['id']; ?>)">×</span>
												 ...
												 <div class="modal-body">
														<form method="post" action="approve_virtualassistant_staff.php" style="display:inline">
															<input type="hidden" name="id" value="<?php echo $virtualassistantResult[$x]['id']; ?>">
															<input type="hidden" name="email" value="<?php echo $virtualassistantResult[$x]['email']; ?>">
															<input type="hidden" name="fullname" value="<?php echo $virtualassistantResult[$x]['fullname']; ?>">
															<input type="hidden" name="account_type" value="1">
															<input type="hidden" name="status" value="1">
															<button type="submit" class="btn-success" style="font-size:12px;">YES, as Virtual Assistant</button>
														</form>
														<form method="post" action="approve_virtualassistant_staff.php" style="display:inline">
															<input type="hidden" name="id" value="<?php echo $virtualassistantResult[$x]['id']; ?>">
															<input type="hidden" name="email" value="<?php echo $virtualassistantResult[$x]['email']; ?>">
															...
															<input type="hidden" name="status" value="1">
															<button type="submit" class="btn-success" style="font-size:12px;">Yes, as Virtual Assistant QC</button>
														</form>
												</div>
											</div>
										</div>
										<div id="declinevirtualassistantmodal<?php echo $virtualassistantResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "declinevirtualassistantmodal" . (string)$virtualassistantResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to decline?</h3>
												 </div>
												 <div class="modal-body">
													<form method="post" action="approve_virtualassistant_staff.php">
														<input type="hidden" name="id" value="<?php echo $virtualassistantResult[$x]['id']; ?>">
														<input type="hidden" name="skype" value="<?php echo $virtualassistantResult[$x]['skype']; ?>">
														<input type="hidden" name="email" value="<?php echo $virtualassistantResult[$x]['email']; ?>">
														<input type="hidden" name="paypal" value="<?php echo $virtualassistantResult[$x]['paypal']; ?>">
														<input type="hidden" name="fullname" value="<?php echo $virtualassistantResult[$x]['fullname']; ?>">
														<input type="hidden" name="status" value="0">
														<button type="submit" class="btn-danger" style="font-size:1rem;">Reject</button>
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "declinevirtualassistantmodal" . (string)$virtualassistantResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									<td>
										<button class="btn-danger" onclick="showModal(<?php echo "deletePendingvirtualassistantModal" . (string)$virtualassistantResult[$x]['id']; ?>)">REMOVE</button>
										<div id="deletePendingvirtualassistantModal<?php echo $virtualassistantResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "deletePendingvirtualassistantModal" . (string)$virtualassistantResult[$x]['id']; ?>)">×</span>
												 ...
												 <div class="modal-body">
													<form method="post" action="remove_virtualassistant_staff.php">
														...
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "deletePendingvirtualassistantModal" . (string)$virtualassistantResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									<td>
										<center><input type="checkbox" class="remove_all" name="remove_all[]" value="<?php echo $virtualassistantResult[$x]['id']; ?>"></center>
									</td>
								</tr>
							<?php }?>
						<?php }else{?>
				...
				<table cellspacing="0">
					<thead>
						<tr>
							...
							<th style="width:6%">Delete</th>
							<th><button class="btn-danger" onclick="showDeleteAllApprovedvirtualassistantModal()">REMOVE ALL</button></th>
						</tr>
					</thead>
					<tbody>
						...
						<?php if(!empty($approvevirtualassistantResult)){ ?>
							<?php for($x = 0; $x < count($approvevirtualassistantResult); $x++){ ?>
								<tr>	
									...
									<td class="score-total"><?php echo $approvevirtualassistantResult[$x]['test_score']; ?>%</td>
									<td>
										<button class="btn-default" onclick="showTestResultApprovedModal(<?php echo $approvevirtualassistantResult[$x]['id']; ?>)">View Test</button>
										<div id="viewApproveTestModal<?php echo $approvevirtualassistantResult[$x]['id']; ?>" class="modal">
											<div class="modal-content" style="width: 45%;">
												 <div class="modal-header">
													<span class="close" style="top: -10px;" onclick="closeModal(<?php echo "viewApproveTestModal" . (string)$approvevirtualassistantResult[$x]['id']; ?>)">×</span>
													<h4 style="margin:0">Test Result: Multiple Choice</h4>
												 </div>
												 <div class="modal-body" style="padding-bottom:24px">
													<p style="margin-top:0;">Score: <span style="color:#18AACF;font-weight:bold;"><?php echo $approvevirtualassistantResult[$x]['test_score']; ?>%</span></p>
									...
									<td>
										<button class="btn-danger" onclick="showModal(<?php echo "deleteApprovedvirtualassistantModal" . (string)$approvevirtualassistantResult[$x]['id']; ?>)">REMOVE</button>
										<div id="deleteApprovedvirtualassistantModal<?php echo $approvevirtualassistantResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "deleteApprovedvirtualassistantModal" . (string)$approvevirtualassistantResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to remove?</h3>
												 </div>
												 <div class="modal-body">
													<form method="post" action="remove_virtualassistant_staff.php">
														<input type="hidden" name="id" value="<?php echo $approvevirtualassistantResult[$x]['id']; ?>">
														<input type="hidden" name="check" value="2">
														<button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "deleteApprovedvirtualassistantModal" . (string)$approvevirtualassistantResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									?>
										<center><input type="checkbox" class="remove_all_approved" name="remove_all_approved[]" value="<?php echo $approvevirtualassistantResult[$x]['id']; ?>" checked></center>
									<?php }else{ ?> 
										<center><input type="checkbox" class="remove_all_approved" name="remove_all_approved[]" value="<?php echo $approvevirtualassistantResult[$x]['id']; ?>"></center>
									<?php } ?>
                                ...
                                <tbody>
                                        <tr>
                                                <td style="width:40%;vertical-align: top;border:0;">
                                                        <small>APPROVE EMAIL</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: VIRTUAL ASSISTANT</strong>
                                                        <br/>
                                                        ...
                                                        <br/>
                                                        <small>APPROVE EMAIL</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: VIRTUAL ASSISTANT QC</strong>
                                                        <br/>
                                                        <form action="save_virtualassistant_approve_email_template.php" method="post">
							                            ...
							                            <br/>
                                                        <small>AUTO CREATE ACCOUNT</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: VIRTUAL ASSISTANT</strong>
                                                        <br/>
                                                        <form action="save_virtualassistant_approve_email_template.php" method="post">
                                                        ...
                                                        <br/>
                                                        <small>AUTO CREATE ACCOUNT</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: VIRTUAL ASSISTANT QC</strong>
                                                        <br/>
                                                        <form action="save_virtualassistant_approve_email_template.php" method="post">
                                                                ...
                                                                <div class="follow-up-emails">
                                                                        ...
                                                                        <form action="save_virtualassistant_followup_email_template.php" method="post">
                                                                                ...
                                                                                &ensp;<select name="type" required>followupEmailTemplate
                                                                                        <option value="1" <?php if($followupEmailTemplate[$d]['type'] == 1) echo "selected"; ?>>VIRTUAL ASSISTANT</option>
                                                                                        <option value="2" <?php if($followupEmailTemplate[$d]['type'] == 2) echo "selected"; ?>>VIRTUAL ASSISTANT QC</option>
                                                                                </select>
...
<!--end email template-->
    ...
    <div id="myModal" class="modal">
        ...
        <div class="modal-content">
            ...
            <embed id="embedPDF" src="<?php //echo "https://topremotestaff.us-east-1.linodeobjects.com/cv/".$virtualassistantResult[$x]['cv']; ?>" type="application/pdf" width="100%" height="600px" />
        </div>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="post" action="remove_all_virtualassistant_staff.php">
					<input type="hidden" name="staff_ids">
					<button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
					<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeDeleteAllvirtualassistantModal()">No</button>
				</form>
			</div>
		</div>
	</div>
