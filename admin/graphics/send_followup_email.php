...
$getStaff = mysqli_query($db, "SELECT * FROM ts_graphics_users WHERE status='1'");
$staff = mysqli_fetch_all($getStaff, MYSQLI_ASSOC);

$getEmailContent = mysqli_query($db, "SELECT * FROM ts_graphics_followup_email_template");
$emailContent = mysqli_fetch_all($getEmailContent, MYSQLI_ASSOC);

$getEmailSent = mysqli_query($db, "SELECT * FROM ts_graphics_email_sent");
...
for($x = 0; $x < count($staff) ; $x++){
	...
	for($y = 0; $y < count($emailContent) ; $y++){
		...
		$checkEmailSent = $db->query($crud->getData('ts_graphics_email_sent', array("user_id","email_template_id"), array($staff_id,$content_id)));
			...
			if ($content_type == $staff_type){
				...
				if($today_timestamp > $checkTimestamp){
					...
					if($email->sendToCompany($staff_email, $subject, $bodyHtml)){
						...
						$sentDataQuery = $crud->add('ts_graphics_email_sent', $sentData);
