...
if(!isset($_GET['tab']) || (isset($_GET['tab']) && $_GET['tab'] == "pending")) {
	$getGraphicsData = mysqli_query($db, "SELECT * FROM ts_graphics_users WHERE status='0' ORDER BY date_created DESC");
	$GraphicsResult = mysqli_fetch_all($getGraphicsData, MYSQLI_ASSOC);
}
if(isset($_GET['tab']) && $_GET['tab'] == "approved") {
	$getGraphicsApproved = mysqli_query($db, "SELECT * FROM ts_graphics_users WHERE status='1' ORDER BY date_updated DESC");
	$approveGraphicsResult = mysqli_fetch_all($getGraphicsApproved, MYSQLI_ASSOC);
}
if(isset($_GET['tab']) && $_GET['tab'] == "mc_questions") {
	$getQuestions = mysqli_query($db, "SELECT * FROM ts_graphics_questions WHERE status='1' ORDER BY date_created DESC");
	$questionResult = mysqli_fetch_all($getQuestions, MYSQLI_ASSOC);
}
if(isset($_GET['tab']) && $_GET['tab'] == "email_template") {
	$getApproveEmailTemplate = mysqli_query($db, "SELECT * FROM ts_graphics_approve_email_template");
	$approveEmailTemplate = mysqli_fetch_all($getApproveEmailTemplate, MYSQLI_ASSOC);

	$getFollowupEmailTemplate = mysqli_query($db, "SELECT * FROM ts_graphics_followup_email_template");
...
<body>
	...
	<h2>Graphics</h2>
	<div id="Graphics" class="tabcontent p-1" style="display: block;">
		<div class="tab">
		<?php
		if(!empty($_GET)){
				...
				<table cellspacing="0">
					<thead>
						<tr>
							...
							<th><button class="btn-danger" onclick="showDeleteAllPendingGraphicsModal()">REMOVE ALL</button></th>
						</tr>
					</thead>
					<tbody>
					<?php if(!isset($_GET['tab']) || $_GET['tab'] == "pending"){ ?>
						<?php if(mysqli_num_rows($getGraphicsData) > 0){ ?>
							<?php for($x = 0; $x < count($GraphicsResult); $x++){ ?>
								<tr>
									<td><?php echo urldecode($GraphicsResult[$x]['fullname']); ?></td>
									<td><?php echo $GraphicsResult[$x]['email']; ?></td>
									<td><?php echo $GraphicsResult[$x]['phone']; ?></td>
									<td><?php echo $GraphicsResult[$x]['skype']; ?></td>
									<td><?php echo $GraphicsResult[$x]['source']; ?></td>
									<td>
                                        <?php if (!empty($GraphicsResult[$x]['portfolio_url'])) {?>
                                            <a href="<?php echo $GraphicsResult[$x]['portfolio_url']; ?>" class="showCV" id="myBtn" target="_blank">Show URL</a>
                                        <?php } ?>
                                    </td>
									<td>
                                        <?php if (!empty($GraphicsResult[$x]['cv'])) {?>
                                            <button class="showCV" onclick="showCVModal('<?php print $GraphicsResult[$x]['cv']; ?>')"  id="myBtn">View CV</button>
                                        <?php } ?>
                                    </td>
									<td class="score-total"><?php echo $GraphicsResult[$x]['test_score']; ?>%</td>
									<td>
										<button class="btn-default" onclick="showTestResultModal(<?php echo $GraphicsResult[$x]['id']; ?>)">View Test</button>
										<div id="viewTestResultModal<?php echo $GraphicsResult[$x]['id']; ?>" class="modal">
											<div class="modal-content" style="width: 45%;">
												 <div class="modal-header">
													<span class="close" style="top: -10px;" onclick="closeModal(<?php echo "viewTestResultModal" . (string)$GraphicsResult[$x]['id']; ?>)">×</span>
													<h4 style="margin:0">Test Result: Multiple Choice</h4>
												 </div>
												 <div class="modal-body" style="padding-bottom:24px">
													<p style="margin-top:0;">Score: <span style="color:#18AACF;font-weight:bold;"><?php echo $GraphicsResult[$x]['test_score']; ?>%</span></p>
													<div id="test_result_<?php echo $GraphicsResult[$x]['id']; ?>"></div>
												</div>
											</div>
										</div>
									</td>
									<td>
										<button class="btn-success" onclick="showModal(<?php echo "approveGraphicsmodal" . (string)$GraphicsResult[$x]['id']; ?>)">APPROVE</button>
										<button class="btn-danger" onclick="showModal(<?php echo "declineGraphicsmodal" . (string)$GraphicsResult[$x]['id']; ?>)">DECLINE</button>
										<div id="approveGraphicsmodal<?php echo $GraphicsResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "approveGraphicsmodal" . (string)$GraphicsResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to approve?</h3>
												 </div>
												 <div class="modal-body">
														<form method="post" action="approve_graphics_staff.php" style="display:inline">
															<input type="hidden" name="id" value="<?php echo $GraphicsResult[$x]['id']; ?>">
															<input type="hidden" name="email" value="<?php echo $GraphicsResult[$x]['email']; ?>">
															<input type="hidden" name="fullname" value="<?php echo $GraphicsResult[$x]['fullname']; ?>">
															<input type="hidden" name="account_type" value="1">
															<input type="hidden" name="status" value="1">
															<button type="submit" class="btn-success" style="font-size:12px;">YES, as GRAPHICS DESIGER</button>
														</form>
														<form method="post" action="approve_graphics_staff.php" style="display:inline">
															<input type="hidden" name="id" value="<?php echo $GraphicsResult[$x]['id']; ?>">
															<input type="hidden" name="email" value="<?php echo $GraphicsResult[$x]['email']; ?>">
															...
															<input type="hidden" name="status" value="1">
															<button type="submit" class="btn-success" style="font-size:12px;">Yes, as GRAPHICS DESIGER QC</button>
														</form>
												</div>
											</div>
										</div>
										<div id="declineGraphicsmodal<?php echo $GraphicsResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "declineGraphicsmodal" . (string)$GraphicsResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to decline?</h3>
												 </div>
												 <div class="modal-body">
													<form method="post" action="approve_graphics_staff.php">
														<input type="hidden" name="id" value="<?php echo $GraphicsResult[$x]['id']; ?>">
														<input type="hidden" name="skype" value="<?php echo $GraphicsResult[$x]['skype']; ?>">
														<input type="hidden" name="email" value="<?php echo $GraphicsResult[$x]['email']; ?>">
														<input type="hidden" name="paypal" value="<?php echo $GraphicsResult[$x]['paypal']; ?>">
														<input type="hidden" name="fullname" value="<?php echo $GraphicsResult[$x]['fullname']; ?>">
														<input type="hidden" name="status" value="0">
														<button type="submit" class="btn-danger" style="font-size:1rem;">Reject</button>
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "declineGraphicsmodal" . (string)$GraphicsResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									<td>
										<button class="btn-danger" onclick="showModal(<?php echo "deletePendingGraphicsModal" . (string)$GraphicsResult[$x]['id']; ?>)">REMOVE</button>
										<div id="deletePendingGraphicsModal<?php echo $GraphicsResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "deletePendingGraphicsModal" . (string)$GraphicsResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to remove?</h3>
												 </div>
												 <div class="modal-body">
													<form method="post" action="remove_graphics_staff.php">
														<input type="hidden" name="id" value="<?php echo $GraphicsResult[$x]['id']; ?>">
														<input type="hidden" name="check" value="1">
														<button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "deletePendingGraphicsModal" . (string)$GraphicsResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									<td>
										<center><input type="checkbox" class="remove_all" name="remove_all[]" value="<?php echo $GraphicsResult[$x]['id']; ?>"></center>
									</td>
								</tr>
							<?php }?>
						<?php }else{?>
				...
				<table cellspacing="0">
					<thead>
						<tr>
							...
							<th style="width:6%">Delete</th>
							<th><button class="btn-danger" onclick="showDeleteAllApprovedGraphicsModal()">REMOVE ALL</button></th>
						</tr>
					</thead>
					<tbody>
					<?php if(!isset($_GET['tab']) || $_GET['tab'] == "approved"){ ?>
						<?php if(!empty($approveGraphicsResult)){ ?>
							<?php for($x = 0; $x < count($approveGraphicsResult); $x++){ ?>
								<tr>
									<td><?php echo urldecode($approveGraphicsResult[$x]['fullname']); ?></td>
									<td><?php echo $approveGraphicsResult[$x]['email']; ?></td>
									<td><?php echo $approveGraphicsResult[$x]['phone']; ?></td>
									<td><?php echo $approveGraphicsResult[$x]['skype']; ?></td>
									<td><?php echo $approveGraphicsResult[$x]['paypal']; ?></td>
									<td>
																		<?php
																			if($approveGraphicsResult[$x]['account_type'] == 1){
																						echo "GRAPHICS DESIGER";
																					}else{
																							echo "GRAPHICS DESIGER QC";
																					}
																			?>
																	</td>
									<td>
										<?php
											echo date('Y-m-d H:i:s', $approveGraphicsResult[$x]['date_updated']);

										?>
									</td>
									<td class="score-total"><?php echo $approveGraphicsResult[$x]['test_score']; ?>%</td>
									<td>
										<button class="btn-default" onclick="showTestResultApprovedModal(<?php echo $approveGraphicsResult[$x]['id']; ?>)">View Test</button>
										<div id="viewApproveTestModal<?php echo $approveGraphicsResult[$x]['id']; ?>" class="modal">
											<div class="modal-content" style="width: 45%;">
												 <div class="modal-header">
													<span class="close" style="top: -10px;" onclick="closeModal(<?php echo "viewApproveTestModal" . (string)$approveGraphicsResult[$x]['id']; ?>)">×</span>
													<h4 style="margin:0">Test Result: Multiple Choice</h4>
												 </div>
												 <div class="modal-body" style="padding-bottom:24px">
													<p style="margin-top:0;">Score: <span style="color:#18AACF;font-weight:bold;"><?php echo $approveGraphicsResult[$x]['test_score']; ?>%</span></p>
													<div id="approved_test_result_<?php echo $approveGraphicsResult[$x]['id']; ?>"></div>
												</div>
											</div>
										</div>
									</td>
									<td>
										<button class="btn-danger" onclick="showModal(<?php echo "deleteApprovedGraphicsModal" . (string)$approveGraphicsResult[$x]['id']; ?>)">REMOVE</button>
										<div id="deleteApprovedGraphicsModal<?php echo $approveGraphicsResult[$x]['id']; ?>" class="modal">
											<div class="modal-content">
												 <div class="modal-header">
													<span class="close" onclick="closeModal(<?php echo "deleteApprovedGraphicsModal" . (string)$approveGraphicsResult[$x]['id']; ?>)">×</span>
													<h3 style="margin:0">Are you sure you want to remove?</h3>
												 </div>
												 <div class="modal-body">
													<form method="post" action="remove_graphics_staff.php">
														<input type="hidden" name="id" value="<?php echo $approveGraphicsResult[$x]['id']; ?>">
														<input type="hidden" name="check" value="2">
														<button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
														<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeModal(<?php echo "deleteApprovedGraphicsModal" . (string)$approveGraphicsResult[$x]['id']; ?>)">No</button>
													</form>
												</div>
											</div>
										</div>
									...
									<?php
										...
										$approvedDate = ($num_days * $per_day) + $approveGraphicsResult[$x]['date_updated'];
										$approvedPlusDays = $approvedDate - $today_timestamp;

										if($approvedPlusDays <= 0){
									?>
										<center><input type="checkbox" class="remove_all_approved" name="remove_all_approved[]" value="<?php echo $approveGraphicsResult[$x]['id']; ?>" checked></center>
									<?php }else{ ?>
										<center><input type="checkbox" class="remove_all_approved" name="remove_all_approved[]" value="<?php echo $approveGraphicsResult[$x]['id']; ?>"></center>
									<?php } ?>
						...
						<?php }else{?>
							<tr>
								<td colspan="12">No approved staff.</td>
							</tr>
						<?php } ?>
					<?php } ?>
					</tbody>
                                ...
                                <tbody>
                                        <tr>
                                                <td style="width:40%;vertical-align: top;border:0;">
                                                        <small>APPROVE EMAIL</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: GRAPHICS DESIGER</strong>
                                                        <br/>
                                                        <form action="save_graphics_approve_email_template.php" method="post">
                                                        ...
                                                        <br/>
                                                        <small>APPROVE EMAIL</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: GRAPHICS DESIGER QC</strong>
                                                        <br/>
                                                        <form action="save_graphics_approve_email_template.php" method="post">
							                            ...
							                            <br/>
                                                        <small>AUTO CREATE ACCOUNT</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: GRAPHICS DESIGER</strong>
                                                        <br/>
                                                        <form action="save_graphics_approve_email_template.php" method="post">
                                                        ...
                                                        <br/>
                                                        <small>AUTO CREATE ACCOUNT</small>&ensp;<strong style="font-size: 12px;">JOB TYPE: GRAPHICS DESIGER QC</strong>
                                                        <br/>
                                                        <form action="save_graphics_approve_email_template.php" method="post">
                                                ...
                                                <td style="width:40%;vertical-align: top;border:0" class="follow-up-email-container">
                                                        <?php for($d = 0; $d < count($followupEmailTemplate); $d++){ ?>
                                                                <div class="follow-up-emails">
                                                                        ...
                                                                        <br/>
                                                                        <form action="save_graphics_followup_email_template.php" method="post">
                                                                                ...
                                                                                &ensp;<select name="type" required>followupEmailTemplate
                                                                                        <option value="1" <?php if($followupEmailTemplate[$d]['type'] == 1) echo "selected"; ?>>GRAPHICS DESIGER</option>
                                                                                        <option value="2" <?php if($followupEmailTemplate[$d]['type'] == 2) echo "selected"; ?>>GRAPHICS DESIGER QC</option>
                                                                                </select>
...
<!--end email template-->
	...
	<div id="deleteAllModal" class="modal">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form method="post" action="remove_all_graphics_staff.php">
    ...
    <div id="myModal" class="modal">
        ...
        <div class="modal-content">
            ...
            <embed id="embedPDF" src="<?php //echo "https://topremotestaff.us-east-1.linodeobjects.com/cv/".$GraphicsResult[$x]['cv']; ?>" type="application/pdf" width="100%" height="600px" />
        </div>

    </div>
	<div id="deleteAllGraphicsModal" class="modal">
		<div class="modal-content">
			<div class="modal-header">
				<span class="close" onclick="closeDeleteAllGraphicsModal()">×</span>
				<h3 style="margin:0">Are you sure you want to remove all?</h3>
			</div>
			<div class="modal-body">
				<form method="post" action="remove_all_graphics_staff.php">
					<input type="hidden" name="staff_ids">
					<button type="submit" class="btn-danger" style="font-size:1rem;">Yes</button>
					<button type="button" class="btn-default" style="font-size:1rem;" onclick="closeDeleteAllGraphicsModal()">No</button>
				</form>
			</div>
		</div>
	</div>
	...
	<script>
		...
		function showDeleteAllApprovedGraphicsModal() {

			var checkedVals = $('#approved .remove_all_approved:checkbox:checked').map(function() {
				return this.value;
			}).get();

			document.getElementById('deleteAllGraphicsModal').style.display = "block";
			$('input[name=staff_ids]').val(checkedVals.join(","));

		}
		function showDeleteAllPendingGraphicsModal() {
		...
		function closeDeleteAllGraphicsModal() {
			document.getElementById('deleteAllGraphicsModal').style.display = "none";
		}
		...
		function openTab(evt, cityName) {
		  ...
		  if(cityName == "approved" || cityName == "pending" || cityName == "mc_questions" || cityName == "email_template" || cityName == "Graphics"){
